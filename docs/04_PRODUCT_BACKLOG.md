...
- v30.3.83: in development — make Play current-hole-first, move compact Round Progress below Add Memory, preserve the complete Quick Scoreboard, and make PWA updates saved-state-aware with explicit version and completion feedback.
- v30.3.84: deliver AI Recap Governance & Content Reliability by recovering and versioning the deployed `round-recap` function source; inventorying but not altering production; consolidating the client and server prompts into one versioned content contract; making deterministic Round facts binding; defining privacy, sensitive-content, unsupported-inference, length, section-omission, and post-freeze publication rules; and adding contradiction checks plus representative content acceptance fixtures. Any server deployment requires separate Product Owner approval.

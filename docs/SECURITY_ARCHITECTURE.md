...
> **Constitutional authority and approved supersession:** This document is subordinate to [The Dye Ledger Constitution v1.0](architecture/THE_DYE_LEDGER_CONSTITUTION_v1.0.md) and the approved [v30.3.75 Identity & Security Decisions](architecture/IDENTITY_AND_SECURITY_DECISIONS_v30.3.75.md). Decision A-001 supersedes the public-read assumption in the Course Library proposal below. Cloud canonical Course Library reads require authenticated, authorized access; unauthenticated access is denied; and previously downloaded course data remains available offline. Implementation changes will occur only in an approved security work package.
...
> **Legacy proposal — not approved for deployment under A-001.** The statements that public cloud access is available and approved courses are publicly readable are superseded. They remain here only to document the proposal that existing SQL and application behavior were designed against.
...
> **Legacy proposal — not approved for deployment under A-001.** In particular, the proposed `anon` SELECT grant conflicts with the authenticated-only cloud Course Library policy. This documentation note does not change the existing migration or production state.
...
The parent course is written before tees, and each tee before its holes. v31.0.05 writes each tee's holes in one conflict-safe batch against the existing `(tee_id, hole_number)` uniqueness contract. Normal sync does not delete/recreate children. `deleteCloudCourseById` explicitly deletes holes, tees, then course, although foreign keys also cascade. Database IDs are text and are generated by database defaults for new browser inserts; existing local/cloud IDs are retained for updates.

v31.0.05 also performs a client-side protected-state check before writing: approved, archived, rejected, legacy, and maintainer catalog rows are never normal browser write targets. This supplements rather than replaces RLS. Draft approval continues to use only the protected `publish_course(text)` RPC after the client verifies maintainer capability. Post-publication reads are scoped to affected course IDs; a full catalog read occurs only through the explicit download/refresh workflow.
...
> **Legacy implementation pending migration.** Operation through the unauthenticated `anon` role conflicts with Decision A-001. The local-first failure behavior remains required: denied cloud access must preserve local course data and immutable Round Course Snapshots.

...
export const deterministicFixtures = [
  ...
  fixture({
    scenario: 'wolf_rotating_points',
    purpose: 'Wolf rotates by played position, resolves declared sides, and settles individual point differentials to zero.',
    selectedGames: [{ key:'wolf', basis:'gross', playerIds:['p1','p2','p3','p4'], pointValue:1, allowLoneWolf:true, allowBlindWolf:true, finalHolesRule:'continue', points:{teamWin:1,opponentsWin:1,loneWolfWin:4,loneWolfLoss:1,blindWolfWin:8,blindWolfLoss:1} }],
    wolfInputs: Object.fromEntries(Array.from({length:18},(_,index)=>[String(index+1),{choice:'partner',partnerPlayerId:['p2','p3','p4','p1'][index%4],declaredAt:`2026-09-08T00:${String(index).padStart(2,'0')}:00Z`} ])),
    expectedInvariants: ['final_settlement_zero_sum','game_payouts_reconcile','save_reload_stable','wolf_settlement_zero_sum'],
    scores: {
      p1: [4,5,3,4,4,5,4,3,4,4,5,3,4,4,5,4,3,4],
      p2: [5,6,4,5,5,6,5,4,5,5,6,4,5,5,6,5,4,5],
      p3: [6,6,4,5,6,6,5,4,5,6,6,4,5,6,6,5,4,5],

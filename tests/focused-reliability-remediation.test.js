...
test('focused setup disclosures, Honors pill, and header-only icon change match acceptance markup', () => {
  ...
  assert.match(manifest, new RegExp(`branding/app-icon-192-${currentVersionRegexEscaped}\\.png`));
  assert.match(manifest, new RegExp(`branding/apple-touch-icon-${currentVersionRegexEscaped}\\.png`));

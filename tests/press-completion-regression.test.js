...
test('UI/save source locks, current icon references, cache paths, and branding docs are consistent', () => {
  ...
  assert.match(html, /rel="apple-touch-icon"[^>]+href="\.\/branding\/apple-touch-icon-v30\.3\.83\.png"/);
  assert.doesNotMatch(html, /<img src="\.\/branding\/app-icon-192\.png" alt="The Dye Ledger"/);
  assert.match(worker, /branding\/apple-touch-icon-v30\.3\.83\.png/);
  assert.match(worker, /the-dye-ledger-v30\.3\.83/);

...
test('v30.3.79 identity is consistent across runtime, cache, manifest, and package metadata', () => {
  assert.equal(pkg.version, '30.3.79');
  assert.equal(manifest.version, 'v30.3.79');
  assert.match(app, /versionNumber:\s*'30\.3\.79'/);
  assert.match(worker, /v30\.3\.79/);
  assert.match(html, /id="appVersionLabel">v30\.3\.79/);
});

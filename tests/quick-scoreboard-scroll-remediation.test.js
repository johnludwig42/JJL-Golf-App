...
test('header, Apple, desktop PWA, favicon, worker cache, and branding documentation agree without changing artwork bytes', () => {
  assert.match(htmlSource, /<img src="\.\/branding\/apple-touch-icon-v30\.3\.99\.png" alt="The Dye Ledger"/);
  assert.equal(count(htmlSource, /rel="apple-touch-icon"/g), 1);
  assert.match(htmlSource, /rel="apple-touch-icon"[^>]+href="\.\/branding\/apple-touch-icon-v30\.3\.99\.png"/);
  assert.doesNotMatch(htmlSource, /brand-mark[\s\S]{0,200}app-icon-192/);

  const expected = [
    ['./branding/app-icon-192-v30.3.99.png', '192x192'],
    ['./branding/app-icon-512-v30.3.99.png', '512x512'],
    ['./branding/apple-touch-icon-v30.3.99.png', '180x180'],
  ];
  ...
  assert.match(worker, /cacheName: 'the-dye-ledger-v30\.3\.99'/);
  assert.match(htmlSource, /favicon-32-v30\.3\.99\.png/);
  assert.match(htmlSource, /favicon-16-v30\.3\.99\.png/);

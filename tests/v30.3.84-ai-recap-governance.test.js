...
test('current metadata and immutable PWA assets remain consistent', () => {
  ...
  for (const name of ['app-icon-192-v31.0.06.png', 'app-icon-512-v31.0.06.png', 'apple-touch-icon-v31.0.06.png', 'favicon-32-v31.0.06.png', 'favicon-16-v31.0.06.png']) {
...
test('More shows exactly the current and four preceding release notes', () => {
  ...
  assert.deepEqual(notes.slice(0, 5).map(note => note.version), ['v31.0.06', 'v31.0.05', 'v31.0.04', 'v31.0.03', 'v31.0.02']);
...
test('current assets refresh and Play exposes the authoritative End Round workflow', () => {
  ...
  for (const asset of ['manifest.json', 'style.css', 'supabase-config.js', 'identity-security.js', 'app.js']) {
    assert.match(html, new RegExp(`${asset.replace('.', '\\.') }\\?v=31\\.0\\.06`));
    assert.match(worker, new RegExp(`${asset.replace('.', '\\.') }\\?v=31\\.0\\.06`));
  }

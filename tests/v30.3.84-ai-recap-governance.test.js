...
test('More shows exactly the current and four preceding release notes', () => {
  ...
  const versions = notes.slice(0, 5).map(note => note.version);
  assert.equal(versions.length, 5);
  assert.equal(versions[0], currentVersionPrefixed);
  assert.equal(new Set(versions).size, 5);
  const numeric = versions.map(version => version.slice(1).split('.').map(Number));
  assert.ok(numeric.every(parts => parts.length === 3 && parts.every(Number.isInteger)));
  assert.ok(numeric.slice(1).every((parts, index) => {
    const prior = numeric[index];
    return parts[0] < prior[0] || (parts[0] === prior[0] && (parts[1] < prior[1] || (parts[1] === prior[1] && parts[2] < prior[2])));
  }));

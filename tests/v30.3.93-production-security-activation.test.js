...
test('current and older clients attribute new Shared Match ownership',()=>{assert.match(app,/created_by:\s*organizerUserId\s*\|\|\s*null/);assert.match(migration,/function public\.stamp_shared_match_owner\(\)/i);assert.match(migration,/before insert on public\.matches/i);assert.match(rollback,/drop trigger if exists stamp_shared_match_owner_before_insert/i);});

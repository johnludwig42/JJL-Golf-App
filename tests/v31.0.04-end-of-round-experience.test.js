...
test('same-version service workers do not produce an update offer', () => {
  const engine = loadLiveEngine();
  assert.equal(engine.shouldOfferServiceWorkerUpdate('31.0.06'), false);
  assert.equal(engine.shouldOfferServiceWorkerUpdate('v31.0.06'), false);
  assert.equal(engine.shouldOfferServiceWorkerUpdate('31.0.06'), true);

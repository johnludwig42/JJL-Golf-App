...
test('v31.0.06 immutable foundation assets remain available', () => {
...
test('Classic remains the safe default and known modes normalize explicitly', () => {
...
test('Play input mode is a device preference and legacy preferences migrate to Classic', () => {
  ...
  assert.equal(engine.getPlayerPreferences(store).schemaVersion, 6);
...
test('shared controller reads the authoritative Round score and stat arrays without copying them', () => {
  ...
  assert.equal(controller.schemaVersion, 2);
